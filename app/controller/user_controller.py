...
def register_admin(data):
    ...
    if user_model.find_admin_by_email(email):
        raise HTTPException(status_code=400, detail="Admin already exists")

    hashed_password = hash_password(data.password)
    admin = user_model.create_admin(data.name, email, hashed_password)
...
def forgot_password(data):
    ...
    user_model.save_otp(email, otp, expiry)
...
def verify_otp(data):
    ...
    user = user_model.verify_otp(email, otp)

    if not user:
        raise HTTPException(status_code=400, detail="Invalid or expired OTP")

    # Clear OTP after verification
    user_model.save_otp(email, None, None)
